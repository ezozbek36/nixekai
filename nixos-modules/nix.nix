{
  ...
  config = lib.mkMerge [
    (lib.mkIf (config.nix.patchLixPipeOperator) {
      nixpkgs.overlays = [
        (final: prev: {
          lixPackageSets = prev.lixPackageSets.extend (f: p: let
    ...
    })
    {
      nix = {
        package =
          if config.nix.implementation == "nix"
          then pkgs.nix
          else if config.nix.implementation == "lix"
        ...
        settings = {
          ...
          experimental-features = [
            ...
            "nix-command"
            (
              if config.nix.implementation == "lix" && !config.nix.patchLixPipeOperator
              then "pipe-operator"
              else "pipe-operators"
            )
          ];
